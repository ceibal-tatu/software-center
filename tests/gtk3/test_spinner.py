...
class SpinnerNotebookTestCase(unittest.TestCase):
    ...
    def test_show_spinner(self):
        ...
        with patch.object(spinner.GLib, 'timeout_add',
    ...
    def test_show_spinner_twice(self):
        """The spinner is not hiden/shown if its already visible."""
        with patch.object(self.obj.spinner_view, "stop_and_hide") as m:
            with patch.object(spinner.GLib, 'timeout_add',
                              lambda t,f: f()):
                self.obj.show_spinner("meep")
                self.obj.show_spinner("baap")
            # ensure that it only called the hide once
            self.assertEqual(m.call_count, 1)
            # and that it updated the text
            self.assertEqual(self.obj.spinner_view.get_text(), "baap")

    def test_show_spinner_with_msg(self):
        """The spinner is shown with the given message."""
        message = 'Something I want to show'
        with patch.object(spinner.GLib, 'timeout_add', lambda *a: None):
    ...
    def test_hide_spinner_before_timeout(self):
        ...
        with patch.object(spinner.GLib, 'source_remove',
        ...
        self.assertTrue(self._fake_timeout_id is None,
            'The timeout should be removed by calling GLib.source_remove')
    ...
    def test_hide_spinner_after_timeout(self):
        ...
        self.assertTrue(self._fake_timeout_id is not None,
            'GLib.source_remove should not be called if already fired.')
        # the content page is shown
        self.assertEqual(self.obj.get_current_page(), self.obj.CONTENT_PAGE)
        # the spinner is stoppped and hidden
        self.assertFalse(self.obj.spinner_view.spinner.get_property('active'))
