...
class Worker(threading.Thread):
    ...
    def verify_server_status(self):
        """ verify that the server we want to talk to can be reached
            this method should be overridden if clients talk to a different
            server than rnr
        """
        try:
...
class BaseApp(SimpleGtkbuilderApp):
    ...
    def login(self, show_register=True):
        ...
        self.sso = get_login_backend(login_window_xid,
...
class SubmitReviewsApp(BaseApp):
    ...
    def _modify_review_is_the_same(self):
        ...
        if (self.review_summary_entry.get_text_length() !=
                ...
                self.review_buffer.get_char_count() !=
                    len(self.orig_review_text)):
            return False
        #compare rating
    ...
    def _get_gwibber_prefs(self):
        send = self.config.reviews_post_via_gwibber
        account_id = self.config.reviews_gwibber_account_id
    ...
    def _save_gwibber_state(self, gwibber_send, account_id):
        self.config.reviews_post_via_gwibber = gwibber_send
        if account_id:
            self.config.reviews_gwibber_account_id = account_id
        self.config.write()
    ...
    def _gwibber_message(self, max_len=140):
        ...
        def _gwibber_message_string_from_data(appname, rating, summary, link):
            ...
            return _("reviewed %(appname)s in Ubuntu: %(rating)s "
                       "%(summary)s %(link)s") % {'appname': appname,
                                                  'rating': rating,
                                                  'summary': summary,
                                                  'link': link}
