...
class SoftwareCenterAgent(GObject.GObject):
    ...
    def query_available_for_me(self, no_relogin=False):
        spawner = SpawnHelper()
        spawner.parent_xid = self.xid
        spawner.ignore_cache = self.ignore_cache
    ...
    def _on_exhibits_data_available(self, spawner, exhibits):
        for exhibit in exhibits:
            ...
            if not hasattr(exhibit, "click_url"):
                exhibit.click_url = ""
            # ensure to fix #1004417 and #1043152
            if exhibit.package_names:
                exhibit.package_names = ",".join(
                    [s.strip() for s in exhibit.package_names.split(",")])
...
if __name__ == "__main__":
    ...
    def _available_for_me(agent, available_for_me):
        print ("_available_for_me: %s" % available_for_me)
