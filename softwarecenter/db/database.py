...
def get_reinstall_previous_purchases_query():
    ...
    :return: a xapian query to get all the apps that are purchased
    """
...
class StoreDatabase(GObject.GObject):
    ...
    def schema_version(self):
        """Return the version of the database layout
           ...
           This is useful to ensure we force a rebuild if it's
    ...
    def get_pkgname(self, doc):
        ...
        if not pkgname:
            ...
            # fallback to the old way (the xapian DB may not yet be rebuilt)
    ...
    def get_xapian_document(self, appname, pkgname):
        """Get the matching xapian document for appname, pkgname.
